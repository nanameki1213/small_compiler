...
statement
  ...
  | TOKEN_ID TOKEN_COLEQ expression
                        { gen_code("store_id", $1); }
  | TOKEN_ID TOKEN_COL  { gen_code("label", $1); }
    statement
  | TOKEN_ID TOKEN_LPAR { args = 0; }
    arglist_opt
  ...
  | TOKEN_IF expression TOKEN_THEN
    ...
    statement else_opt
  | TOKEN_WHILE       { strcpy(label1, new_label());
  ...
  | TOKEN_REPEAT  {  strcpy(label1, new_label());
                     gen_code("label", label1);
                  }
